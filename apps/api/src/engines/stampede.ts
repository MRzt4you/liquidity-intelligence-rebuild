...
type WalletAction = { wallet: string; mint: string; side: 'BUY' | 'SELL'; sol: number; ts: number; signature: string; isLaunch?: boolean };
...
export class StampedeEngine {
  ...
  observe(a: WalletAction) {
    if (!a.wallet || !a.mint || !a.signature) return;
    const now = Date.now();
    let t = this.tokens.get(a.mint);
    if (!t) {
      t = { mint: a.mint, symbol: a.mint.slice(0, 6), createdAt: 0, lastSeen: now, buys: 0, sells: 0, volumeSol: 0, buySol: 0, sellSol: 0, buyers: new Set(), sellers: new Set(), wallets: new Set(), score: 0, risk: 'UNKNOWN' };
      this.tokens.set(a.mint, t);
    }
    t.lastSeen = Math.max(t.lastSeen, a.ts || now);
    if (a.isLaunch && a.ts > 0) t.createdAt = t.createdAt ? Math.min(t.createdAt, a.ts) : a.ts;
  ...
  snapshot(limit = 100) {
    return [...this.tokens.values()].map(t => {
      const age = t.createdAt > 0 ? Math.max(0, Date.now() - t.createdAt) : null;
      ...
      const fresh = age !== null ? (age < 5 * 60_000 ? 20 : age < 30 * 60_000 ? 12 : 5) : 0;
      ...
      return { mint:t.mint, symbol:t.symbol, ageMs:age, ageVerified:age !== null, buys:t.buys, sells:t.sells, volumeSol:+t.volumeSol.toFixed(4), buySol:+t.buySol.toFixed(4), sellSol:+t.sellSol.toFixed(4), buyers:t.buyers.size, sellers:t.sellers.size, wallets:t.wallets.size, rotationWallets:rotation.wallets, rotationOrigins:rotation.origins, score, risk, lastSeen:t.lastSeen };
