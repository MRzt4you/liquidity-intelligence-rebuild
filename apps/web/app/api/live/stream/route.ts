...
export async function GET(request:Request){
 ...
 const source=(i:number)=>USE_SOLAMI&&i===0?'solami-blur':'solana-pubsub';
 ...
 const connectUpstream=async():Promise<boolean>=>{
   ...
   try{
     ...
     if(!ws){scheduleReconnect(`websocket upgrade unavailable: ${source(providerIndex)}`);return false}
     ...
     ws.addEventListener('error',()=>{if(remote===ws)scheduleReconnect(`${source(providerIndex)} upstream error`)});
     ws.addEventListener('close',()=>{if(remote===ws)scheduleReconnect(`${source(providerIndex)} upstream closed`)});
 ...
 server.addEventListener('message',(event:any)=>{try{const msg=JSON.parse(String(event.data));if(msg?.type==='ping')send({type:'pong',events,lastEventAt:lastEventAt||null,source:source(providerIndex)})}catch{}});
 ...
 send({type:'ready',source:source(providerIndex),providers:providers.length,mode:'event-verified-persistent'});
 healthTimer=setInterval(()=>{if(closed)return;const sinceEvent=lastEventAt?Date.now()-lastEventAt:null;const sinceConnect=connectedAt?Date.now()-connectedAt:null;send({type:'health',state:remote?(lastEventAt&&sinceEvent!<30000?'LIVE':'CONNECTED_NO_EVENTS'):'RECONNECTING',source:source(providerIndex),events,lastEventAt:lastEventAt||null,silentMs:sinceEvent,connectedMs:sinceConnect,reconnecting,providerIndex,providers:providers.length})},5000);
