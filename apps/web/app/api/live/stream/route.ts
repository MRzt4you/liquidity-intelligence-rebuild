...
export async function GET(request:Request){
 ...
 const connectUpstream=async():Promise<boolean>=>{if(closed)return false;const url=providers[providerIndex]||providers[0];const solami=USE_SOLAMI&&providerIndex===0;try{
   ...
   if(!ws){fail(`websocket upgrade unavailable: ${source(providerIndex)}`);return false}
   remote=ws;ws.accept();reconnecting=false;reconnectDelay=1000;
   send({type:'upstream',state:'connected',source:source(providerIndex),provider:solami?'solami-blur':url,events,lastEventAt:lastEventAt||null});
   ws.addEventListener('message',event=>{if(closed||remote!==ws)return;try{const msg=JSON.parse(String(event.data));
     if(solami){
       if(msg?.type==='error'||msg?.error){fail(String(msg?.error?.message||msg?.message||'Solami stream error'));return}
       const parsed=normalizeSolami(msg); if(!parsed)return;
       events++; lastEventAt=Date.now();
       send({type:'event',event:parsed,eventType:parsed.event,source:'solami-blur',events,lastEventAt});
       return;
     }
     if(msg.id===1||msg.id===2){if(msg.error){fail(String(msg.error.message||'subscription error'));return}if(msg.result!==undefined)send({type:'subscription',id:msg.id,result:msg.result});return}
     if(msg.method==='logsNotification'){events++;lastEventAt=Date.now();send({type:'event',eventType:'ONCHAIN_LOG',source:'solana-pubsub',events,lastEventAt,log:msg.params});send({type:'logs',data:msg.params})}
   }catch{}});
   ws.addEventListener('error',()=>{if(remote===ws)fail(`${source(providerIndex)} upstream error`)});
   ws.addEventListener('close',()=>{if(remote===ws)fail(`${source(providerIndex)} upstream closed`)});
   if(solami){for(const filter of [{type:'token_create'},{type:'swap',dex:'pumpswap'},{type:'liquidity',dex:'pumpswap'},{type:'pool_create',dex:'pumpswap'}])try{ws.send(JSON.stringify(filter))}catch{return false}}
   else try{PROGRAMS.forEach((program,i)=>ws.send(JSON.stringify({jsonrpc:'2.0',id:i+1,method:'logsSubscribe',params:[{mentions:[program]},{commitment:'processed'}]})))}catch{closeRemote();return false}
   return true;
 }catch(error){fail(error instanceof Error?error.message:'upstream fetch failed');return false}};
 server.addEventListener('message',(event:any)=>{try{const msg=JSON.parse(String(event.data));if(msg?.type==='ping')send({type:'pong',events,lastEventAt:lastEventAt||null})}catch{}});
